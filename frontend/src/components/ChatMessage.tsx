...
interface ConfidenceLabel { text: string; classes: string; dot: string }
...
function getConfidenceLabel(confidence: number | undefined, content: string): ConfidenceLabel | null {
  ...
  if (confidence >= 0.6) return { text: 'High confidence',              classes: 'text-emerald-400 bg-emerald-500/[0.08] border-emerald-500/20', dot: 'bg-emerald-400' }
  if (confidence >= 0.3) return { text: 'Moderate confidence',          classes: 'text-yellow-400 bg-yellow-500/[0.08] border-yellow-500/20',   dot: 'bg-yellow-400' }
...
const ChatMessage: React.FC<Props> = ({ message, isAdmin = false, onRetry, onFollowUp }) => {
  const [copied, setCopied] = useState(false)
  const isUser = message.role === 'user'
  const confidenceLabel = isAdmin && !isUser && !message.isError
